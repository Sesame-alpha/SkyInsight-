...
const CALLSIGN_MAP = {
    ...
    'MSR': 'EgyptAir', 'KQA': 'Kenya Airways', 'RAM': 'Royal Air Maroc',
    'MAU': 'Air Mauritius', 'DTA': 'TAAG Angola Airlines', 'BOT': 'Air Botswana'
};
...
async function buildAndTrainModel() {
    // Create a simple sequential model: 3 inputs (altitude, velocity, hour)
    const model = tf.sequential();
    model.add(tf.layers.dense({ units: 8, activation: 'relu', inputShape: [3] }));
    model.add(tf.layers.dense({ units: 4, activation: 'relu' }));
    model.add(tf.layers.dense({ units: 1 })); // output: delay in minutes
    model.compile({ optimizer: 'adam', loss: 'meanSquaredError' });

    // Generate synthetic training data: (alt_ft, vel_knots, hour) -> delay_min
    const numSamples = 500;
    const inputs = [];
    const outputs = [];
    for (let i = 0; i < numSamples; i++) {
        const alt = Math.random() * 40000; // 0-40000 ft
        const vel = Math.random() * 500;    // 0-500 kt
        const hour = Math.random() * 24;
        // Delay formula: lower alt & lower vel = higher delay, plus peak hours
        let delay = (40000 - alt) / 40000 * 30 + (500 - vel) / 500 * 20;
        if (hour > 16 || hour < 6) delay += 10; // night/evening congestion
...
function mapCallsignToAirline(callsign) {
    ...
    for (const [prefix, airline] of Object.entries(CALLSIGN_MAP)) {
        if (upper.startsWith(prefix)) return airline;
    }
    return null;
}
...
async function updateAlertPanel() {
    ...
    for (const flight of filtered) {
        ...
        const confidence = Math.floor(Math.random() * 20) + 80; // mock confidence 80-99%
...
function updateCongestionIndex() {
    ...
    const badge = `<span class="congestion-badge" style="background:${level==='High'?'#FEE2E2':'#E8F0FE'}; color:${level==='High'?'#C0392B':'#2C7DA0'};">${level} (${count} flights)</span>`;
    document.getElementById('congestionBadge').innerHTML = badge;
}
...
function exportPDF() {
    ...
    doc.text(`Live Flights: ${currentFlights.length}`,20,40);
...
async function refreshDashboard() {
    ...
    updateKPIs();
    await updateAlertPanel(); // uses ML predictions
    updateMapMarkers();
    updateEtaTable();
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    refreshInterval = setInterval(refreshDashboard, 20000);
});
